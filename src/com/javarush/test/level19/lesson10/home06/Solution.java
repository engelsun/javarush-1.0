...
public class Solution {
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        List<String> listOfChangeLines = new ArrayList<>();
        for (String s :
                listOfLines)
        {
            ...
            for (int i = 0; i < arrayOfWords.length; i++)
            {
                for (Map.Entry<Integer, String> entry :
                        map.entrySet())
                {
                    if (arrayOfWords[i].equals(String.valueOf(entry.getKey())))
                    {
                        arrayOfWords[i] = entry.getValue();
                    }
                }
                line += arrayOfWords[i] + " ";
            }
            listOfChangeLines.add(line);
        }
