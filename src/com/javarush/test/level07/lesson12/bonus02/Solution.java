...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> listMore = new ArrayList<String>();
        for (int i = 0; i < list.size(); i++)
        {
            String s = list.get(i);
            if(s.length() % 2 == 0) listMore.add(s.concat(" " + s));
            if(s.length() % 2 != 0) listMore.add(s.concat(" " + s + " " + s));
        }

        for (int i = 0; i < listMore.size(); i++)
        {
            System.out.println(listMore.get(i));
        }
    }
}
